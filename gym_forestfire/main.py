...
import gym_forestfire.agents.bcq as bcq
import gym_forestfire.agents.ddpg as ddpg
...
if __name__ == "__main__":  
    ...
    parser.add_argument("--max_timesteps", default= 25000, type=int, help="Max time steps to run environment")
    ...
    parser.add_argument("--load_model", default="default", help="Model load file name, \"\" doesn't load, \"default\" uses file_name")
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if done:
            ...
            episode_time_list.append(episode_num)
            # Reset environment
            state, done = env.reset(), False
            episode_reward = 0
            episode_timesteps = 0
...
#TD3 is the algorithm used
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if done:
            ...
            print(
                f"Total T: {t + 1} Episode Num: {episode_num + 1} Episode T: {episode_timesteps} Reward: {episode_reward:.3f}")
            td3_reward_list.append(episode_reward)
            td3_episode_nums.append(episode_num)
            # Reset environment
            state, done = env.reset(), False
            episode_reward = 0
            episode_timesteps = 0
...
plt.plot(episode_time_list, reward_list, label= 'DDPG' , linestyle= '--' ,color = 'grey' )
plt.plot(td3_episode_nums, td3_reward_list, label= 'TD3' ,color = 'black')
plt.xlim(0, td3_episode_nums[-1])
plt.ylim(-1000,0)
